...
def main():
    ...
    try:
        ...
        if "The security token included in the request is invalid" in error_message:
            ...
            print(
                "ERROR: Problem calling AWS Secrets Manager: {}".format(error_message)
            )
        sys.exit(1)

    config_data = yaml.load(response["SecretBinary"], Loader=yaml.FullLoader)
    config_data["terraform"] = json.loads(response["SecretBinary"])["terraform"]
    ...
    with open("accounts.json.j2") as in_template:
        template = jinja2.Template(in_template.read())
